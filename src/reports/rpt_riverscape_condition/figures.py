...
def statistics(gdf: gpd.GeoDataFrame, tot_area: pint.Quantity) -> dict[str, pint.Quantity]:
    ...
    statistic_fields = ["segment_area", "centerline_length", "elevated_ratio", "lf_agriculture", "lf_developed", "access_fldpln_extent"]
    unique_columns = ~gdf.columns.duplicated()
    subset_df = RSGeoDataFrame(gdf.loc[:, unique_columns].loc[:, statistic_fields].copy())
    # Calculate totals
