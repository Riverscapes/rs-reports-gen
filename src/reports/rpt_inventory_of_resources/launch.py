...
def main() -> list[str] | None:
    ...
    Environment variables that can be set:
        For all reports:
            DATA_ROOT - Path to the outputs folder. A subfolder rpt-inventory-of-resources will be created if it does not exist (REQUIRED)
        ...
        Report-specific variables:
            RNS_AOI_GEOJSON - path to the input geojson file for rpt-inventory-of-resources (optional)
    ...
    if not report_name:
        report_name = geojson_file.stem.replace(' ', '_') + " - Inventory_of_Resources"
    ...
    # ── Build args ────────────────────────────────────────────────────
    args = [
        Path(data_root / "rpt-inventory-of-resources" / report_name.replace(" ", "_")),
        geojson_file,
        report_name,
        "--unit_system",
        unit_system,
