...
def test_get_system_units_converts_imperial_to_si(fresh_meta):
    """Foot should resolve to the explicit SI unit when the system is SI."""
    quantity = 1 * ureg.foot

    result = fresh_meta.get_system_units(quantity)
...
def test_get_system_units_converts_si_to_imperial(fresh_meta):
...
def test_get_system_units_no_conversion_necessary(fresh_meta):
    """Meter should resolve to the explicit imperial unit when the system is imperial."""
    quantity = 1 * ureg.meter
    ...
    result2 = fresh_meta.get_system_units(quantity2)

    assert result2.units == ureg.foot
    assert result2.magnitude == pytest.approx(1.0)


def test_get_system_units_returns_original_when_missing_mapping(fresh_meta):
